  df1 <- as.data.frame(read.csv(file="../../rawdata/ebwt.stats-Bioinf-PubChemSDFCompounds.txt",head=TRUE,sep=","))
  ...
  pdf(file="uk-new-pubchemsdf.pdf",height=5,width=7)
  ...
  print(ggplot(data=df1, aes(x=Blocksize, y=Size, group=Algorithm, colour=Algorithm)) +
      ...
      ggtitle("Kompression von 50 PDB-Dateien") +  # Set title
      theme())
